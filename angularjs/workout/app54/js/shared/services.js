...
angular.module('app')
  .provider('WorkoutService', function(){
    ...
    this.$get = ['WorkoutPlan', 'Exercise', '$http', '$q', '$resource', 
        function(WorkoutPlan, Exercise, $http, $q, $resource){
      ...
      service.getWorkout = function (name) {
        return $q.all([service.Exercises.query().$promise, 
            $http.get(collectionsUrl + '/workouts/' + name, {params: {apiKey, apiKey}})])
            .then(function(response){
              ...
              .then(function (original) {
                  if (original) {
                      ...
                      workoutToSave.exercises = workoutToSave.exercises.map(function (exercise) { 
                          return { name: exercise.details.name, duration: exercise.duration } });
